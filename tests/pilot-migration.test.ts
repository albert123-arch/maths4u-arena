...
import { taskSchema, createTaskVersion, publicVersion, versionInclude } from "../src/lib/content";
...
test("populated migration and publication preserve existing accounts, tasks, attempts and grades", { timeout: 180000 }, async () => {
  ...
  try {
    ...
    const version = await client.$transaction(tx => createTaskVersion(tx, admin, taskSchema.parse({ visibility: "PRIVATE", texts: [{ locale: "en", title: "Existing task", statement: "Existing content", solution: "Existing solution" }], parts: [{ kind: "MANUAL", maxPoints: 4, texts: [{ locale: "en", prompt: "Explain", answer: "Existing answer", rubric: "Existing rubric" }] }] })));
    const part = await client.taskPart.findFirstOrThrow({ where: { versionId: version.id } });
    ...
    await client.attempt.create({ data: { userId: student.id, workVersionId: work.versions[0].id, number: 1, status: "GRADED", expiresAt: new Date(), submittedAt: new Date(), gradedAt: new Date(), answers: { create: { partId: part.id, response: { text: "Student work" }, review: { create: { reviewerId: admin.id, points: 3, comment: "Preserve this grade" } } } } } });
    async function snapshot() {
      const [tables] = await sql.query<mysql.RowDataPacket[]>("SHOW TABLES");
      const result: Record<string, string[]> = {};
      for (const row of tables) {
        const name = String(Object.values(row)[0]);
        if (name === "_prisma_migrations") continue;
        assert.match(name, /^[a-zA-Z_][a-zA-Z0-9_]*$/);
        const [rows] = await sql.query<mysql.RowDataPacket[]>(`SELECT * FROM \`${name}\``);
    ...
    assert.deepEqual(after.lessontask, []);
    delete after.lessontask;

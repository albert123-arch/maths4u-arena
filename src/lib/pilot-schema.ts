...
const texts = z.array(z.object({ locale, title: z.string().min(1).max(191), description: z.string().max(10000).default(""), body: z.string().max(100000).default(""), examples: z.string().max(100000).default("") })).min(1).max(2)
...
export const bundleSchema = z.object({ format: z.enum(["maths4u-pilot-v1", "maths4u-pilot-v2"]), selection: key, sections: z.array(sectionSchema).min(1).max(2),
  assets: z.array(z.object({ key: z.string().regex(/^[a-f0-9]{40}$/), file: z.string().regex(/^assets\/[a-f0-9]{64}\.png$/), sha256: z.string().regex(/^[a-f0-9]{64}$/),
    size: z.number().int().positive().max(8 * 1024 * 1024), mimeType: z.literal("image/png"), role: z.enum(["STATEMENT", "SOLUTION", "MARK_SCHEME", "HINT", "TEACHER"]),
...
export function validatePilot(input: unknown) {
  ...
  for (const section of bundle.sections) {
    ...
    for (const row of section.records) {
      ...
      for (const text of row.material.texts) for (const field of ["statement", "hint", "solution", "markScheme", "teacherNote"] as const) {
        const role = ({ statement: "STATEMENT", hint: "HINT", solution: "SOLUTION", markScheme: "MARK_SCHEME", teacherNote: "TEACHER" } as const)[field];
    ...
    for (const text of section.lesson.texts) if (renderContent(text.body + text.examples).includes('class="katex-error"') || /<img\b/i.test(text.body + text.examples)) throw new Error("PILOT_THEORY_REQUIRES_REVIEW");
  }

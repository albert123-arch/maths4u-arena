...
import { nextHostPacedQuestion } from "@/lib/host-paced";
...
export async function POST(_request: Request, { params }: RouteContext) {
  ...
  try {
    const result = await nextHostPacedQuestion(code);
    ...
    console.error("Host-paced next question failed", error instanceof Error ? error.message : "Unknown error");

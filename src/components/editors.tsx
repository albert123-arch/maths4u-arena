  ...
  return <><Heading title={id ? t("Редактировать задачу", "Edit problem") : t("Новая задача", "New problem")} subtitle={t("Формулы: \\(x^2\\) или $$x^2$$. Поддерживаются безопасный HTML и таблицы.", "Formulas: \\(x^2\\) or $$x^2$$. Safe HTML and tables are supported.")} />
    ...
    <div className="card"><Form label={t("Сохранить задачу", "Save problem")} submit={async () => {
      ...
      {task.parts.map((part, pi) => <section className="card" key={pi}><div className="row spread"><h3>{t("Часть ", "Part ")}{pi + 1}</h3>{task.parts.length > 1 && <button type="button" className="quiet" onClick={() => update(d => { d.parts.splice(pi, 1); })}>{t("Убрать", "Remove")}</button>}</div>
        ...
        <div className="field-grid" style={{ marginTop: 16 }}>{part.texts.map((pt, ti) => <div className="stack" key={pt.locale}><strong>{pt.locale.toUpperCase()}</strong>{(["prompt", "answer", "rubric"] as const).map(key => <label key={key}>{key === "prompt" ? t("Условие части", "Part prompt") : key === "answer" ? t("Ответ для ученика после раскрытия", "Answer shown after release") : t("Критерии оценивания", "Marking criteria")}
          <textarea value={pt[key]} onChange={e => update(d => { d.parts[pi].texts[ti][key] = e.target.value; })} /></label>)}</div>)}</div>
      ...
      <details><summary>{t("Темы, источник и экзаменационные данные", "Topics, source and exam details")}</summary><div className="field-grid">
        {(["source", "syllabus", "examBoard", "examSession", "paper", "questionNumber"] as const).map(key => <label key={key}>{({ source: t("Источник", "Source"), syllabus: t("Программа", "Syllabus"), examBoard: "Exam board", examSession: t("Сессия", "Session"), paper: "Paper", questionNumber: t("Номер вопроса", "Question number") })[key]}<input value={task[key] ?? ""} onChange={e => update(d => { d[key] = e.target.value; })} /></label>)}
